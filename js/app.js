...
import { initI18n, setLanguage, t, getLang, onLanguageChange, SUPPORTED_LANGS, LANG_NAMES } from './i18n.js';
...
function renderCard(code, idx) {
  ...
  card.querySelector('[data-remove]')?.addEventListener('click', () => removeSlot(idx));
...
function createSelector(idx, selectedCode) {
  ...
  const selectedCountry = selectedCode ? getAllCountries().find((c) => c.code === selectedCode) : null;
  trigger.innerHTML = `
  ...
  function updateHighlight(newIdx) {
    if (flatItems[highlighted]) flatItems[highlighted].classList.remove('highlighted');
    highlighted = newIdx;
...
function closeAllDropdowns() {
  ...
  document.querySelectorAll('.selector-trigger.open').forEach((t) => t.classList.remove('open'));
